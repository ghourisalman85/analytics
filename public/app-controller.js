...
class AppController {
  ...
  registerServiceWorker() {
    ...
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.register(globalUrl+'/service-worker.js')
      .catch((err) => {
  ...
  _subscriptionUpdate(subscription) {
    ...
    //samad_code finish
    var json_1 = JSON.stringify(subscription); 
    $.ajax({
        url: 'insert.php?fun=insert_browser_data',
        type:'post',
        dataType:'json',
        data:{"data_browser":json_1},
  ...
  showErrorMessage(title, message) {
      console.log("title error "+title+" title message "+message);
